...
import { SITE_VERSION, SITE_VERSION_LABEL, AUTH_ENGINE_VERSION_TAG } from '../../config/version';
...
export const InteractiveTerminal: React.FC = () => {
  ...
  const initialHistory: CommandOutput[] = [
    {
      ...
      response: `Successfully installed tc_auth-${SITE_VERSION} fastapi-0.110 pydantic-2.6 sqlalchemy-2.0 PyJWT-2.8 bcrypt-4.1`,
      status: 'success',
      timestamp: '00:00:01',
    },
    {
  ...
  const executeCommand = (cmdStr: string) => {
    ...
    if (lower.startsWith('tc-auth otp send')) {
      ...
      response = (
        <div className="space-y-1 text-zinc-300">
          ...
          <div className="bg-zinc-950 p-2 rounded text-[11px] font-mono text-indigo-300">
            {JSON.stringify({ aid: 1, sid: 42, role: 'superadmin', exp: 1787404800, iss: AUTH_ENGINE_VERSION_TAG }, null, 2)}
          </div>
        </div>
      );
      ...
      response = <div className="text-emerald-400">Requirement already satisfied: tc_auth=={SITE_VERSION} in /usr/local/lib/python3.11</div>;
    } else {
      response = (
    ...
    <div className="rounded-2xl border border-zinc-800 bg-zinc-950/90 shadow-2xl overflow-hidden font-mono text-xs">
      ...
      >
        <div className="text-zinc-500 text-[11px] leading-relaxed">
          Welcome to tc_auth {SITE_VERSION_LABEL} interactive shell. Type commands below or click quick actions.

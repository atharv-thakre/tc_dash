          ...
          <div className="space-y-1">
            ...
            {isDocsOpen && (
              <div className="space-y-1 pt-1">
                {/* Explorer Link - Uniform with other nav items */}
                <button
                  ...
                  <div className="flex items-center gap-2.5">
                    <FolderTree
                      className={cn(
                        ...
                        activePath.startsWith('/documents') ? 'text-white' : 'text-slate-400 dark:text-zinc-500'
                      )}
                    />
                    <span>Explorer</span>
                  </div>
                  ...
                  <button
                    ...
                    className={cn(
                      ...
                      activePath.startsWith('/docs/api')
                        ? 'bg-indigo-50 dark:bg-zinc-900/80 text-indigo-600 dark:text-indigo-300 dark:border-zinc-800 font-bold'
                        : 'text-slate-600 dark:text-zinc-400 hover:bg-slate-100 dark:hover:bg-zinc-900/60 hover:text-slate-900 dark:hover:text-zinc-100'
                    )}
                  >
                    <div className="flex items-center gap-2.5">
                      <Terminal className="w-4 h-4 text-slate-400 dark:text-zinc-500 group-hover:scale-110 transition-transform" />
                      <span>REST API</span>
                    </div>
                    <div className="flex items-center gap-1.5">
                      <span className="text-[10px] font-mono px-1.5 py-0.2 rounded-md bg-slate-100 dark:bg-zinc-800 text-slate-500 dark:text-zinc-400">
                        {API_DOCS.length}
                      </span>
                      {isApiOpen ? <ChevronDown className="w-3.5 h-3.5 text-slate-400" /> : <ChevronRight className="w-3.5 h-3.5 text-slate-400" />}
                    </div>
                  </button>

                  {isApiOpen && (
                    <div className="pl-4 space-y-0.5 border-l border-slate-200 dark:border-zinc-800 ml-3.5 py-1">
                      {API_DOCS.map((doc) => {
                        ...
                        return (
                          <button
                            ...
                            className={cn(
                              'block w-full text-left px-2.5 py-1 text-[11px] rounded-lg transition-all font-mono truncate cursor-pointer',
                              isActive
                                ? 'text-indigo-600 dark:text-indigo-400 font-bold bg-indigo-50 dark:bg-zinc-900'
                ...
                <div className="space-y-0.5">
                  ...
                  >
                    <div className="flex items-center gap-2.5">
                      <FileCode2 className="w-4 h-4 text-slate-400 dark:text-zinc-500 group-hover:scale-110 transition-transform" />
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      {isLibOpen ? <ChevronDown className="w-3.5 h-3.5 text-slate-400" /> : <ChevronRight className="w-3.5 h-3.5 text-slate-400" />}
                    </div>
                  </button>

                  {isLibOpen && (
                    <div className="pl-4 space-y-0.5 border-l border-slate-200 dark:border-zinc-800 ml-3.5 py-1 max-h-64 overflow-y-auto">
                      {LIBRARY_DOCS.map((doc) => {
                        ...
                        return (
                          <button
                            ...
                            onClick={() => handleNavClick(targetPath)}
                            className={cn(
                              'block w-full text-left px-2.5 py-1 text-[11px] rounded-lg transition-all truncate cursor-pointer',
                              isDocActive
